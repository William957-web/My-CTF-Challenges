...
P = 1289837564986090927380812179078126226643568303
S = 182873473
PROBE_SALT = 3
...
class OracleSolver:
    ...
    def solve_digest(self) -> bytes:
        ...
        while True:
            ...
            if z == 0:
                run_low = bit
                self.log(f"found terminating run boundary at bit {run_low}")
                break
        ...
        if run_low == 256:
            digest_int = recovered_low
            self.log(
                f"digest reconstruction complete without tail probe | ones={digest_int.bit_count()} "
                f"| get={self.used_get}/128 flip={self.used_flip}/128"
            )
            return digest_int.to_bytes(32, "big")

        # After z == 0, the remaining live token is exactly a contiguous run of 1s
        # starting at run_low. We can measure its length using the same fixed seed S:
        # flipping the current least-significant 1 advances the oracle value from
        # bit*S to (bit+1)*S, and flipping the last 1 yields 256*S.
        run_len = 1
        current_bit = run_low
        while True:
            if self.used_flip >= 128:
                raise RuntimeError("flip budget exhausted while measuring tail run")
            self.flip_bit(current_bit)
            if self.used_get >= 128:
                raise RuntimeError("proof budget exhausted while measuring tail run")
            y = self.oracle(S)
            if y == 256 * S:
                self.log(f"tail run length = {run_len}")
                break
            expected = (current_bit + 1) * S
            if y != expected:
                raise RuntimeError(
                    f"tail run measurement mismatch at bit {current_bit}: got {y}, expected {expected}"
                )
            run_len += 1
            current_bit += 1
            if current_bit >= 256:
                raise RuntimeError("tail run exceeded digest width")

        remaining = ((1 << run_len) - 1) << run_low
        token = recovered_low | remaining
        digest_int = token & ((1 << 256) - 1)
        self.log(
...
def run_challenge_attempt(
    ...
    for round_index in range(1, 17):
        ...
        if verbose:
            print(
                f"[attempt {attempt}] round {round_index:02d}: received server suffix",
                file=sys.stderr,
                flush=True,
            )
